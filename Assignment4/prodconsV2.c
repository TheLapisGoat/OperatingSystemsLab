...
//Using semaphores to solve the producer consumer problem (Using a mutex lock)
...
int main(int argc, char * argv[]) {
    ...
    semid = semget(IPC_PRIVATE, 1, 0777|IPC_CREAT);        //Creating a semaphore for consumer (No. of items in buffer)

    //Initializing semaphore
    semctl(semid, 0, SETVAL, 1);

    //Initializing the sembufs pop and vop so that pop is used for P(semid) and vop is used for V(semid)
    pop.sem_num = 0;
    vop.sem_num = 0;
    pop.sem_flg = 0;
    vop.sem_flg = 0;
